...
function cwr_enqueue_styles() {
    ...
    wp_enqueue_style(
        ...
        array(), 
        filemtime(plugin_dir_path(__FILE__) . 'style.css') // cache-busting
    );
}
...
function cwr_get_user_referral_link($user_id) {
    ...
    if ($referral_code === 'error-generating-code') {
        error_log('CWR Referral: Failed to generate referral link for user ID ' . $user_id);
        return home_url();
    }
...
function cwr_wallet_exists($user_id) {
    global $wpdb;
    return (bool) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}cwr_wallets WHERE user_id = %d",
            $user_id
        )
    );
}
...
function cwr_add_verify_instruction_message() {
    ...
    if (in_array($redirect_to, ['/referral-program', urlencode('/referral-program'), 'verify-referral-code']) && $is_not_logged_in && $is_my_account) {
